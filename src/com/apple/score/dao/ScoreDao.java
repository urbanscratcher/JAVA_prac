...
public class ScoreDao // implements WorkDiv;
{
	...
	private String ADD_FILE = StringUtil.getPath("score.csv");
			...
			while ((data = br.readLine()) != null) // 파일 내용의 다음 줄이 null일 때까지
			{
				...
				if (!"".equals(data) || data.length() != 0) // 빈칸이아니거나 길이가 0이 아닌경우
				{
					...
					outVO = new ScoreVO(ScoreArray[0], // memId
							...
							Integer.parseInt(ScoreArray[3]), // score;
	...
	{
		int cnt = 1;
		List<ScoreVO> scVoList = new ArrayList<ScoreVO>();
		ScoreVO inVO = (ScoreVO) dto;

		for (int i = 0; i < score.size(); i++) 
		{
			ScoreVO outVo = score.get(i);
			if (	inVO.getMemId().equals(outVo.getMemId())) 
			{
				scVoList.add(outVo);
			}
		}
...
//		}
	...
	public ScoreVO getInputData(Scanner dataScanner) {
		...
		} else // 모든 데이터 입력
		{
			...
			outVO = new ScoreVO(ScoreArray[0], ScoreArray[1], ScoreArray[2], Integer.parseInt(ScoreArray[3]), ScoreArray[4],
					ScoreArray[5]);
		}
