...
public class AttendanceDao implements WorkDiv {
	...
	private List<AttendanceVO> attendanceList; //2. 파일 올려서 리스트에 올려놓고 시작한다.
	private final String SAVE_FILE_NM = "memberData";
	...
	public AttendanceDao() {
		Properties prop = PropertiesUtils.readProperties();
		   String path = prop.getProperty("root");    //키값이 루트인 파일을 읽어서 가져옴
	        ...
	        System.out.println("==============");
	        ADD_FILE = path;
			attendanceList = readFile(ADD_FILE);
	...
	public List<AttendanceVO> readFile(String path){
		...
		try {
			...
			while ((data = br.readLine()) != null) { //읽어들이면 된다.
				...
				// jamesol@paran.com,이상무,2020-07-06,010-1234-5678
				LOG.debug("data:" + data);
				data = data.trim();
				if ("".equals(data) || data.length() == 0) {
					continue;
				}
				//202007150701_001,java 프로젝트_01,첫번째 프로젝트_01,0,20200715,james
				String[] dateArray = data.split(",");
				// LOG.debug("memberArray.length:"+memberArray.length);
	...
	public int doSave(DTO dto) {
		...
		if (isSuccess == true)flag = 1; //성공적으로 작성하면 트루를 받는다.
		
		LOG.info("isSuccess:" + isSuccess);//찍어본다.
		for (AttendanceVO vo : attendanceList) {
			//LOG.debug(vo);
	...
	public int doDelete(DTO dto) {
		...
		//파일에서 삭제
		if(flag !=0)this.doSaveFile(attendanceList);
